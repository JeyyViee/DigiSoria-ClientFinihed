...
// Similar logic for videos and music can be implemented
  ...
  const handlePress = async () => {
    setHiding(true); // Start loading
    try {
      if (product.hidden) {
        await onUnhide(); // Execute the unhide function
      } else {
        await onHide(); // Execute the hide function
      }
    } finally {
      setHiding(false); // Stop loading
    }
  };  
    ...
    {product.musics.map((musicUrl, index) => (
      ...
      <View style={styles.hideContainer}>
        <TouchableOpacity
          ...
          style={[
            styles.actionButton,
            { backgroundColor: product.hidden ? '#EE82EE' : '#9966CC' },
          ]}
          disabled={hiding}
        >
          {hiding ? (
            ...
            <Text style={styles.actionButtonText}>
              {product.hidden ? '🔓 Unhide Item' : '🔒 Hide Item'}
            </Text>
          )}
        </TouchableOpacity>
      </View>
     ...
     <View style={styles.hideContainer}>
        <TouchableOpacity
          ...
          style={[
            styles.actionButton,
            { backgroundColor: featuring ? '#FFD700' : product.isFeatured ? '#FFA500' : '#808080' },
          ]}
          disabled={featuring}
        >
          {featuring ? (
            ...
            <Text style={styles.actionButtonText}>
              {product.isFeatured ? '🌟 Unfeature Item' : '⭐ Feature Item'}
            </Text>
          )}
        </TouchableOpacity>
      </View>
