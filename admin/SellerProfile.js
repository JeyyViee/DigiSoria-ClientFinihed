...
const SellerProfile = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchSellers = async () => {
      ...
      const sellerQuery = query(usersCollection, where('role', '==', 'Seller'));
  ...
  const renderSellerDetails = ({ item }) => {
    ...
    return (
      <Card style={[styles.card, !isVerified && styles.cardNotVerified]}>
        <Card.Content>
          ...
          <Paragraph style={styles.productsCount}>Products Listed: {item.productsCount}</Paragraph>
          <Paragraph style={styles.servicesCount}>Services Offered: {item.servicesCount}</Paragraph>
        </Card.Content>
        <Card.Actions>
          <Button
  ...
  return (
    <ImageBackground source={backgroundImage} style={styles.backgroundImage}>
      <View style={styles.container}>
        <Text style={styles.heading}>Sellers Details</Text>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={renderSellerDetails}
...
const styles = StyleSheet.create({
  ...
  cardNotVerified: {
    backgroundColor: '#f5f5f5', // Lighter background for unverified sellers
  },
  profilePicContainer: {
    alignItems: 'center',
    marginBottom: 10,
